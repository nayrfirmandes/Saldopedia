        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 sm:p-8 mb-8">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
            <div className="flex items-start gap-3">
              <div className="w-12 h-12 rounded-full flex items-center justify-center overflow-hidden bg-gray-50 dark:bg-gray-700 flex-shrink-0">
                <Image 
            ...
            >
              {(isProofUploaded || uploadSuccess) ? (
                <>
                  ...
                  {t('orderInstructions.statusVerifying')}
                </>
              ) : order.status === "pending" || order.status === "pending_proof" ? (
                <>
                  <Clock className="w-4 h-4" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <div className="text-lg font-bold text-blue-600 dark:text-blue-400">
                  <FormatSaldo amount={order.amount_idr} />
                </div>
              </div>
            </div>
          </div>
        ...
        {!isBuy && (isPayPal || isSkrill) && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 sm:p-8 mb-6">
            {(uploadSuccess || isProofUploaded) ? (
              <>
                <div className="flex items-start gap-3 mb-6">
                  <div className="flex-shrink-0 w-12 h-12 sm:w-14 sm:h-14 bg-blue-100 dark:bg-blue-900/20 rounded-2xl flex items-center justify-center">
                    <Clock className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 dark:text-blue-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-1">
                      {t('orderInstructions.verifyingPayment')}
                    </h2>
                    <p className="text-sm text-blue-600 dark:text-blue-400 mb-2">
                      {t('orderInstructions.proofReceived')}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-4 mb-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">{serviceLabel} {t('orderInstructions.amount')}</div>
                      <div className="text-lg font-bold text-gray-900 dark:text-white">{formatUSD(order.amount_input)}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('orderInstructions.youWillReceive')}</div>
                      <div className="text-lg font-bold text-green-600 dark:text-green-400"><FormatSaldo amount={order.amount_idr} /></div>
                    </div>
                  </div>
                </div>

                <div className="border-l-2 border-blue-500 pl-4 py-2">
                  <div className="text-sm font-medium text-gray-900 dark:text-white mb-2">{t('orderInstructions.whatsNext')}</div>
                  <ol className="list-decimal list-inside space-y-1.5 text-sm text-gray-600 dark:text-gray-400">
                    <li>{t('orderInstructions.adminVerifying')}</li>
                    <li>{t('orderInstructions.emailNotification')}</li>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-medium text-red-800 dark:text-red-300 mb-1">
                          {t('orderInstructions.paypalWarningTitle')}
                        </p>
                        <p className="text-sm text-red-700 dark:text-red-400">
                          {t('orderInstructions.paypalWarningDesc')}
                        </p>
                      </div>
                    </div>
                  </div>
