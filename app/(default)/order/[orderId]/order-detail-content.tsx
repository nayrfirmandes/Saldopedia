...
import { ArrowLeft, CheckCircle, Clock, XCircle, AlertTriangle, Copy, Check, ExternalLink, Wallet, CreditCard, Mail, Phone, User, Hash, Globe, FileText, Tag, RefreshCw, Loader2 } from 'lucide-react';
...
export default function OrderDetailContent({ order: initialOrder }: OrderDetailContentProps) {
  const { t } = useLanguage();
  ...
  const isSellOrder = order.transaction_type === 'sell';
  const isCryptoOrder = order.service_type === 'cryptocurrency';
  const hasPayout = !!order.nowpayments_payout_id;
  const hasPayment = !!order.nowpayments_payment_id;
  const isNotFinal = order.status !== 'completed' && order.status !== 'failed' && order.status !== 'cancelled' && order.status !== 'expired';
  
  const needsPayoutCheck = isBuyOrder && isCryptoOrder && hasPayout && isNotFinal;
  const needsPaymentCheck = isSellOrder && isCryptoOrder && hasPayment && isNotFinal;
  const needsStatusCheck = needsPayoutCheck || needsPaymentCheck;

  const checkStatus = useCallback(async () => {
    if (!needsStatusCheck || isCheckingPayout) return;
    
    setIsCheckingPayout(true);
    try {
      const endpoint = needsPayoutCheck ? '/api/nowpayments/check-payout' : '/api/nowpayments/check-payment';
      ...
      if (data.success && data.order) {
        if (data.order.status === 'completed' || data.order.status === 'failed' || data.order.status === 'expired') {
          router.refresh();
        } else {
          setOrder(prev => ({
            ...prev,
            payout_status: data.order.payoutStatus || prev.payout_status,
            payout_hash: data.order.payoutHash || prev.payout_hash,
            payment_status: data.order.paymentStatus || prev.payment_status,
            actually_paid: data.order.actuallyPaid?.toString() || prev.actually_paid,
          }));
        }
      }
  ...
  }, [needsStatusCheck, needsPayoutCheck, isCheckingPayout, order.order_id, router]);
  ...
  const statusConfig = {
    ...
    confirmed: { color: 'text-blue-600 dark:text-blue-400', bgColor: 'bg-blue-100 dark:bg-blue-900/20', text: t('orderPages.detail.status.confirmed') || 'Confirmed', icon: Clock },
    ...
    failed: { color: 'text-red-600 dark:text-red-400', bgColor: 'bg-red-100 dark:bg-red-900/20', text: t('orderPages.detail.status.failed') || 'Failed', icon: XCircle },
          ...
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center overflow-hidden border border-gray-100 dark:border-gray-800 ${
                  order.status === 'expired' || order.status === 'cancelled' ? 'opacity-50' : ''
                }`}>
                  <Image 
                    src={getServiceIconSrc(order.service_type, order.crypto_symbol)} 
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                    {isCheckingPayout ? (
                      <>
                        ...
                        <span>{t('orderPages.detail.checkingPayout') || 'Checking status...'}</span>
                      </>
                    ) : (
                      <>
                        <RefreshCw className="h-3 w-3" />
                        <span>{t('orderPages.detail.autoChecking') || 'Auto-checking'}</span>
                      </>
                    )}
                  </div>
                )}
