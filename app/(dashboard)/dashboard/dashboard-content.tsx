...
import { ArrowUpRight, ArrowDownLeft, Plus, Clock, ChevronRight, Wallet, Send, Coins } from 'lucide-react';
...
type DashboardContentProps = {
  ...
  userPointTransactions: PointTransaction[];
  pointsBalance: number;
};

export default function DashboardContent({ user, userOrders, userDeposits, userWithdrawals, userTransfers, userPointTransactions, pointsBalance }: DashboardContentProps) {
  const { t } = useLanguage();
  const orders = userOrders;
  const deposits = userDeposits;
  const withdrawals = userWithdrawals;
  const transfers = userTransfers;
  const pointTransactions = userPointTransactions || [];
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4">
          ...
          <Link href="/dashboard/points" className="group inline-flex items-center gap-2 text-gray-500 dark:text-gray-400 hover:text-amber-600 dark:hover:text-amber-400 transition-colors">
            <Coins className="h-4 w-4 text-amber-500" />
            <span className="text-sm">{pointsBalance.toLocaleString('id-ID')} {t('dashboard.pointsUnit')}</span>
          </Link>
        </div>
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {allActivity.slice(0, 5).map((item) => {
              if (item.type === 'order') {
                ...
                const iconSrc = getServiceIconSrc(order.service_type, order.crypto_symbol);
                  ...
                  <Link key={`order-${order.id}`} href={targetUrl} className="block -mx-2 px-2 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden border border-gray-100 dark:border-gray-800 ${
                        ...
                        <Image 
                          src={iconSrc} 
                          alt={order.crypto_symbol || order.service_type} 
                          width={28} 
                          height={28} 
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className={`text-sm font-medium ${amountColorClass}`}>
                          <FormatSaldo amount={order.amount_idr} showSign={isExpiredOrCancelled ? 'none' : (isSell ? 'positive' : 'negative')} />
                        </p>
                      </div>
                    </div>
                  </Link>
