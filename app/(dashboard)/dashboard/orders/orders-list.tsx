...
import { Clock, Plus, ArrowLeft, Wallet, SlidersHorizontal, ArrowUpRight, ArrowDownLeft, Send, Coins } from 'lucide-react';
import { formatIDR } from '@/lib/formatters';
import { FormatSaldo } from '@/components/format-saldo';
import { useLanguage } from '@/contexts/language-context';
import { getServiceIconSrc } from '@/lib/get-service-icon';
...
export default function OrdersList({ orders: initialOrders, deposits: initialDeposits, withdrawals: initialWithdrawals, transfers: initialTransfers, pointTransactions: initialPointTransactions, currentPage: initialPage, totalPages: initialTotalPages, currentFilter, counts: initialCounts }: OrdersListProps) {
  ...
  const pointTransactions = initialPointTransactions || [];
  ...
  const totalAll = counts.all + counts.deposit + (counts.withdrawal || 0) + (counts.transfer || 0) + (counts.points || 0);
  ...
  const tabs = [
    ...
    { id: 'pending_proof', label: t('dashboardPages.orders.filterPendingProof'), count: counts.pending_proof || 0 },
    ...
    { id: 'points', label: t('dashboardPages.orders.filterPoints'), count: counts.points || 0 },
  ];
  ...
  type ActivityItem = 
    ...
    | { type: 'points'; data: PointTransaction; timestamp: number };
  ...
  const allCombinedActivity: ActivityItem[] = currentFilter === 'all' ? [
    ...
    ...pointTransactions.map(pt => ({
      ...
      data: pt,
      timestamp: (pt as any).created_at_raw || 0
    }))
  ].sort((a, b) => b.timestamp - a.timestamp) : [];

  const pendingProofCombinedActivity: ActivityItem[] = currentFilter === 'pending_proof' ? [
    ...orders.map(order => ({
      type: 'order' as const,
      data: order,
      timestamp: (order as any).created_at_raw || 0
  ...
  const visiblePendingProofActivity = pendingProofCombinedActivity.slice(0, visibleItems);
  const hasMorePendingProof = visibleItems < pendingProofCombinedActivity.length;
  ...
  const visiblePoints = pointTransactions.slice(0, visibleItems);
  const hasMorePoints = visibleItems < pointTransactions.length;
  ...
  return (
    <div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-800 pt-6">
        ...
        {currentFilter === 'all' ? (
              ...
              {visibleCombinedActivity.map((item, index) => {
                if (item.type === 'order') {
                  ...
                  const statusConfig: Record<string, { text: string; color: string }> = {
                    ...
                    verifying: { text: t('dashboard.orderStatus.verifying'), color: 'text-blue-600 dark:text-blue-400' },
                  ...
                  const targetUrl = (order.status === 'pending' || order.status === 'processing' || order.status === 'pending_proof' || order.status === 'verifying')
                  ...
                  const iconSrc = getServiceIconSrc(order.serviceType, order.cryptoSymbol);
                  ...
                  return (
                    <Link key={`order-${order.id}`} href={targetUrl} className="block -mx-2 px-2 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden border border-gray-100 dark:border-gray-800 ${
                          isExpiredOrCancelled ? 'opacity-50' : ''
                        }`}>
                          <Image 
                            src={iconSrc} 
                            alt={order.cryptoSymbol || order.serviceType} 
                            width={28} 
                            height={28} 
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className={`text-sm font-medium ${amountColorClass}`}>
                            <FormatSaldo amount={order.amountIdr} showSign={isExpiredOrCancelled ? 'none' : (isSell ? 'positive' : 'negative')} />
                          </p>
                        </div>
                      </div>
                    </Link>
                  ...
                  const depositStatusConfig: Record<string, { text: string; color: string }> = {
                    ...
                    pending_proof: { text: t('dashboard.depositStatus.pendingProof'), color: 'text-orange-600 dark:text-orange-400' },
                    ...
                    <Link key={`deposit-${deposit.id}`} href={`/dashboard/deposits/${deposit.depositId}`} className="block -mx-2 px-2 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                          ...
                          <Wallet className={`h-5 w-5 ${isExpiredOrRejected ? 'text-gray-400 dark:text-gray-500' : 'text-green-600 dark:text-green-400'}`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 dark:text-white">
                    ...
                    <Link key={`withdrawal-${withdrawal.id}`} href={`/dashboard/withdrawals/${withdrawal.withdrawalId}`} className="block -mx-2 px-2 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                          ...
                          <ArrowDownLeft className={`h-5 w-5 ${isRejected ? 'text-gray-400 dark:text-gray-500' : 'text-red-600 dark:text-red-400'}`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 dark:text-white">
                  ...
                  const getPointDescription = () => {
                    if (pt.type === 'redeem') {
                      const pointsAbs = Math.abs(pt.amount);
                      const idrAmount = pointsAbs * 0.5;
                      return t('dashboard.points.redeemDesc')
                        .replace('{points}', pointsAbs.toLocaleString('id-ID'))
                        .replace('{idr}', idrAmount.toLocaleString('id-ID'));
                    }
                    if (pt.type === 'referral_bonus') {
                      return t('dashboard.points.descReferralBonus');
                    }
                    if (pt.type === 'referred_bonus') {
                    ...
                    if (pt.type === 'adjustment') {
                      return pt.description || t('dashboard.points.descAdjustment');
                    }
                  ...
                  return (
                    <Link key={`points-${pt.id}`} href={`/dashboard/points/${pt.id}`} className="block -mx-2 px-2 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                          isPositive ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-gray-100 dark:bg-gray-800'
                        }`}>
                          <Coins className={`h-5 w-5 ${
                            isPositive ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400 dark:text-gray-500'
                          }`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 dark:text-white">
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            {mounted && (
                              <>
                                {description && <span className="text-gray-300 dark:text-gray-600">-</span>}
                                <span>{pt.createdAt}</span>
                              </>
                            )}
                          </div>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <p className={`text-sm font-medium ${
                            isPositive ? 'text-amber-600 dark:text-amber-400' : 'text-gray-500 dark:text-gray-400'
                          }`}>
                            {isPositive ? '+' : ''}{pt.amount.toLocaleString('id-ID')} {t('dashboard.points.pointsUnit')}
                          </p>
                        </div>
                      </div>
                    </Link>
        ...
        ) : currentFilter === 'pending_proof' ? (
          pendingProofCombinedActivity.length === 0 ? (
            <div className="py-12 text-center">
              <p className="text-gray-500 dark:text-gray-400 mb-4">{t('dashboardPages.orders.noPendingProof')}</p>
            </div>
          ) : (<>
            <div className="divide-y divide-gray-100 dark:divide-gray-800">
              {visiblePendingProofActivity.map((item, index) => {
                if (item.type === 'order') {
                  const order = item.data;
                  const serviceLabel = 
                    order.serviceType === 'cryptocurrency' ? order.cryptoSymbol :
                  ...
                  const status = statusConfig[order.status] || { text: t('dashboard.orderStatus.pendingProof'), color: 'text-orange-600 dark:text-orange-400' };

                  const targetUrl = `/order/instructions/${order.orderId}`;

                  const amountColorClass = isSell 
                    ? 'text-green-600 dark:text-green-400' 
                    : 'text-red-600 dark:text-red-400';
                  const amountSign = isSell ? '+' : '-';
                  ...
                  return (
                    <Link key={`order-${order.id}`} href={targetUrl} className="block -mx-2 px-2 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className={`text-sm font-medium ${amountColorClass}`}>
                            <FormatSaldo amount={order.amountIdr} showSign={isSell ? 'positive' : 'negative'} />
                          </p>
                        </div>
                      </div>
                    </Link>
                  ...
                  const deposit = item.data as Deposit;
                  const methodLabel = deposit.method === 'bank_transfer' 
                    ? `Bank ${deposit.bankCode}` 
                    : deposit.bankCode || 'E-Wallet';
                  
                  const status = { text: t('dashboard.depositStatus.pendingProof'), color: 'text-orange-600 dark:text-orange-400' };
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className="text-sm font-medium text-green-600 dark:text-green-400">
                            <FormatSaldo amount={deposit.amount} showSign="positive" />
                          </p>
                        </div>
                      </div>
                    </Link>
            ...
            {hasMorePendingProof && (
              <div className="pt-6 text-center">
                <button
                  ...
                  className="px-6 py-2 text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"
                >
                  {t('dashboardPages.orders.loadMore')} ({pendingProofCombinedActivity.length - visibleItems} {t('dashboardPages.orders.remaining')})
                </button>
              </div>
            )}
          </>)
          ...
          pointTransactions.length === 0 ? (
            <div className="py-12 text-center">
              <p className="text-gray-500 dark:text-gray-400 mb-4">{t('dashboardPages.orders.noPoints')}</p>
              <Link
                href="/dashboard/referral"
                className="inline-flex items-center gap-2 px-4 py-2 bg-amber-600 hover:bg-amber-700 dark:bg-amber-500 dark:hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-colors"
              >
                <Coins className="h-4 w-4" />
                {t('dashboardPages.orders.earnPoints')}
              </Link>
            </div>
          ) : (<>
            <div className="divide-y divide-gray-100 dark:divide-gray-800">
              {visiblePoints.map((pt) => {
                ...
                return (
                  <Link key={pt.id} href={`/dashboard/points/${pt.id}`} className="block -mx-2 px-2 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                        isPositive ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-gray-100 dark:bg-gray-800'
                      }`}>
            ...
            {hasMorePoints && (
              <div className="pt-6 text-center">
                <button
                  ...
                  className="px-6 py-2 text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"
                >
                  {t('dashboardPages.orders.loadMore')} ({pointTransactions.length - visibleItems} {t('dashboardPages.orders.remaining')})
                </button>
              </div>
            )}
          </>)
