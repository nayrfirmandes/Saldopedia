...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`NOWPayments Payout IPN: Received callback`);
    console.log(`   Payout ID: ${payoutId}, Batch ID: ${batch_withdrawal_id}`);
    console.log(`   Status: ${status}, Currency: ${currency}, Amount: ${amount}`);
    console.log(`   Address: ${address}`);
    console.log(`   Unique External ID: ${unique_external_id}`);
    ...
    if (unique_external_id) {
      result = await sql`
        ...
        SET 
          ...
          updated_at = NOW()
        WHERE order_id = ${unique_external_id}
        RETURNING *
      `;
      if (result.length > 0) {
        console.log(`Found order by unique_external_id (order_id): ${unique_external_id}`);
      }
    }

    if (result.length === 0 && payoutId) {
      result = await sql`
        UPDATE orders
        SET 
          payout_status = ${status},
      ...
      `;
      if (result.length > 0) {
        console.log(`Found order by payout_id: ${payoutId}`);
      }
    }

    if (result.length === 0 && batch_withdrawal_id) {
      result = await sql`
        UPDATE orders
        SET 
          payout_status = ${status},
          ...
          payout_error = ${error || null},
          updated_at = NOW()
        WHERE nowpayments_payout_id = ${batch_withdrawal_id.toString()}
        RETURNING *
      `;
      if (result.length > 0) {
        console.log(`Found order by batch_withdrawal_id: ${batch_withdrawal_id}`);
      }
    }

    if (result.length === 0) {
      console.error(`NOWPayments Payout IPN: Order not found for any ID`);
      console.error(`   Tried: payout_id=${payoutId}, batch_id=${batch_withdrawal_id}, external_id=${unique_external_id}`);
      return NextResponse.json(
